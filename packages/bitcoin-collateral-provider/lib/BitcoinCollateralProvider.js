...
export default class BitcoinCollateralProvider extends Provider {
  ...
  async createManySigs (tx, address, cols, expirations, period) {
    ...
    for (let i = 0; i < cols.length; i++) {
      ...
      inputsToSign.push({ inputTxHex: col.txRaw, index: i, vout: col.colVout, outputScript: colOutputScript })
      addresses.push(address)
    }

    console.log('inputsToSign, addresses, tx, lockTime, isSegwit', inputsToSign, addresses, tx, lockTime, isSegwit)
